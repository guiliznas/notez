...
import { enhanceNote, summarizeNotes } from "../services/geminiService";
...
const GroupDetailPage: React.FC = () => {
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      const notesTexts = messages.map((m) => m.text);
      const summary = await summarizeNotes(notesTexts);
  ...
  const handleArchive = () => {
    if (id && window.confirm("Deseja arquivar este grupo?")) {
      archiveGroup(id);
      navigate("/");
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 max-w-md mx-auto relative">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center sticky top-0 z-30 shadow-sm h-16">
        <button
          onClick={() => navigate(-1)}
          className="mr-3 p-1 -ml-2 rounded-full hover:bg-gray-100 flex-shrink-0"
        >
        ...
        {!isEditingTitle && (
          <div className="flex gap-2">
            <button
              ...
              title="Resumir notas do grupo com IA"
            >
        ...
        {messages.map((msg) => {
          if (msg.isSystem) {
            return (
              <div
                key={msg.id}
                className="w-full text-center py-2 opacity-50 mb-3"
              >
            ...
            <SwipeableItem
              ...
              rightContent={
                <span className="text-white font-bold flex items-center gap-2 ml-auto">
                  Excluir <TrashIcon className="w-5 h-5" />
                </span>
              }
                  ...
                  <div className="flex gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      onClick={async (e) => {
                        ...
                        if (enhanced !== msg.text)
                          updateMessage(msg.id, enhanced);
                      }}
